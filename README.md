...
Maintainers can run the **Test or publish release** GitHub Actions workflow manually. It runs macOS UI regressions, tests natively on Apple silicon and Intel, then builds and verifies the universal package without publishing it. Pushing an annotated upstream version tag that matches `CFBundleShortVersionString`, such as `v0.22.1`, runs the same gates, records GitHub build-provenance and SBOM attestations, and publishes the verified ZIP, DMG, SBOM, and checksums as a GitHub Release.
