...
Maintainers can run the **Test or publish release** GitHub Actions workflow manually. It runs macOS UI regressions, tests natively on Apple silicon and Intel, then builds and verifies the universal package without publishing it. Pushing an annotated upstream version tag that matches `CFBundleShortVersionString`, such as `v0.17.3`, runs the same gates and publishes the verified ZIP, DMG, and checksums as a GitHub Release.
